...
Description:
    ...
    No memory: unlike llm_app_promptMatchedSet.py and llm_app_promptBaseline.py, each of
    the 14 questions is asked in its OWN fresh conversation -- the persona and background are
    resent in full before every single question, and no prior question/answer history carries
    over between questions. each question's 3 alternatives are drawn independently at random from the
    full attribute combination space (see random_profiles()), NOT the choice sets the paired
    human respondent actually saw.

Usage:
    export OPENAI_API_KEY=sk-...
    python3 llm_app_promptNoMemory.py --limit 5      # smoke test
    python3 llm_app_promptNoMemory.py

Input: demographics from `data/messaging_app/demographics/app_real.csv`; choice sets are randomly generated
Output: `data/messaging_app/raw answers/sm_app_{model}_temp{temperature}_promptNoMemory.csv`
"""
...
def run_respondent(client, model, temperature, resp_id, demo_row, resp_choice_sets):
    ...
    for ques in range(1, N_QUESTIONS + 1):
        ...
        # no memory: a fresh conversation for every question, persona+background resent each
        # time, instead of one conversation accumulating history across all 14 questions.
        # order unchanged (persona-first), unlike ChoiceRandomizedNoMemoryOrder -- this variant
        # isolates the memory axis on its own.

...
Description:
    ...
    Shuffled order: each question's 3 alternatives are the exact same choice set the paired human
    respondent saw (see `data/messaging_app/raw answers/app_real.csv`). the display order of the 3 alternatives is randomized each time
    (independent of their original alt 1/2/3 numbering)

Usage:
    export OPENAI_API_KEY=sk-...
    python3 llm_app_promptShuffledOrder.py --limit 5      # smoke test
    python3 llm_app_promptShuffledOrder.py                

Input: demographics from `data/messaging_app/demographics/app_real.csv`, choice sets from `data/messaging_app/raw answers/app_real.csv`
Output: `data/messaging_app/raw answers/sm_app_{model}_temp{temperature}_promptShuffledOrder.csv`
"""
...
def build_question(alt_rows):
    ...
    order each time (independent of their original alt 1/2/3 numbering); the "no app" option
    always stays fixed at position NONE_ALT (4).
    """

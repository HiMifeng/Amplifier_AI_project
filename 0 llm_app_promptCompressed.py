...
Usage:
    ...
    python3 llm_app_promptCompressed.py --limit 5      # smoke test
    python3 llm_app_promptCompressed.py

Input: demographics from `data/messaging_app/demographics/app_real.csv`; choice sets are randomly generated
Output: `data/messaging_app/raw answers/sm_app_{model}_temp{temperature}_promptCompressed.csv`
"""
...
def default_output_csv(model, temperature, dry_run=False):
    ...
    return study_dir / "raw answers" / f"sm_app_{model_label}_temp{temperature:g}_{VARIANT_NAME}{suffix}"
...
# Same logic as llm_app_promptBaseline.py: 3 alternatives per question are drawn
# independently at random from the full attribute combination space, unrelated to what the
# paired human respondent actually saw.
ATTRIBUTE_LEVELS = {
...
def build_background():
    ...
    return (
        ...
        "1. Accessibility: mobile only (smartphone/tablet app, not available on desktop) or web accessible (also usable via a browser on desktop/laptop).\n"
        "2. Authentication: simple (login with username and password), two-factor (login with username, password, and a one-time SMS or email code), or multi-factor (login with username, password, a one-time SMS or email code, and a fingerprint or hardware token), from least to most secure.\n"
        "3. Customisation level: low (adjust basic settings like security and notification preferences), medium (adjust basic settings and organize chats, e.g. chat lists and pinned conversations), or high (adjust basic settings, organize chats, and customize themes and appearance).\n"
        "4. Video calls: one-on-one only, or multi-person (group calls supported).\n"
        "5. Percentage of friends already using the app: the share of your friends, out of all your friends, who already use it.\n"
